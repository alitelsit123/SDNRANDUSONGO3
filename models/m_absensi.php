...
class Absensi{
     ...
     public function create($data) {
      $db = $this->mysqli->conn;
      $db->query("INSERT INTO absensi VALUES(null,'".$data['nama_siswa']."','".$data['sakit']."','".$data['izin']."','".$data['alpha']."','".$data['tahun']."','".$data['semester']."')") or die ($db->error);
     ...
     public function filter($data) {
      ...
      $q = $db->query("SELECT * FROM absensi ".($resultQuery ? 'WHERE '.$resultQuery:'')) or die ($db->error); 
      return $q; 
     }

     public function getTahun($data) {
      $db = $this->mysqli->conn;
      $q = $db->query("SELECT tahun FROM absensi where nama_siswa='$data' group by tahun order by tahun asc") or die ($db->error); 
      return $q; 
     }
     public function filterBySiswa($data) {
      $db = $this->mysqli->conn;
      $q = $db->query("SELECT * FROM absensi where nama_siswa=$data") or die ($db->error); 
      return $q; 
     }
     ...
     public function filterGet($data) {
      ...
      $resultQuery = implode(' AND ', $filters);
      $q = $db->query("SELECT * FROM absensi ".'WHERE '.$resultQuery.' LIMIT 1') or die ($db->error); 
      $result = null;
